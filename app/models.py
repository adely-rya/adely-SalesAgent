...
class VCProfile(Base):
    ...
    # NULL means unknown / not verified.  A missing source is not evidence of
    # non-provision, so support flags deliberately have no False default.
    recruiting_support: Mapped[bool | None] = mapped_column(nullable=True)
    sales_support: Mapped[bool | None] = mapped_column(nullable=True)
    marketing_support: Mapped[bool | None] = mapped_column(nullable=True)
    pr_support: Mapped[bool | None] = mapped_column(nullable=True)
    branding_support: Mapped[bool | None] = mapped_column(nullable=True)
    creative_support: Mapped[bool | None] = mapped_column(nullable=True)
    video_support: Mapped[bool | None] = mapped_column(nullable=True)
    creative_support_level: Mapped[int | None] = mapped_column(nullable=True)
