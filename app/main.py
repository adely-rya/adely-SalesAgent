...
def main() -> int:
    ...
    if args.replay and (not args.scoring_only or args.topic or args.discovery_cache):
        parser.error('--replay requires --scoring-only and cannot combine with --topic or --discovery-cache')
    if args.mode == 'daemon' and (args.scoring_only or args.topic or args.discovery_cache or args.replay):
        parser.error('--scoring-only and --topic require run-once')
    try:
    ...
    try:
        if args.mode == 'daemon':
            ...
            return 0
        return 0 if asyncio.run(run_pipeline(settings, scoring_only=args.scoring_only, topics=args.topic, discovery_cache=args.discovery_cache, replay=args.replay)) else 1
